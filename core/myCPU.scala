...
//-------------IF----------
    ...
    val PCJumpD   = Mux(pre_decoder_jr.asBool,(BranchR1D + _if2id.io.InstrD(31,20)),immUJ + _if2id.io.PCD )// 分支指令均需要读寄存器并且进行比较,由于我是在解码阶段即判断到底需不需要跳转，所以需要用id 阶段的pc进行运算
...
commit_bru_reg := Mux(_cfu.io.StallE.asBool && commit_bru_reg,!_cu.io1.commit_cache_ins,commit_bru_reg)
    ...
    val access_stage1_sram_valid =  0.U//!((inst_buffer.point_write_en.asBool && (!inst_buffer.empty.asBool || (inst_buffer.empty.asBool && inst_write_en =/= 0.U))) || inst_buffer.point_flush.asBool) && !_cp0.io.exception.asBool
...
inst_tlb_exceptionM := Mux(_cfu.io.FlushM.asBool,0.U,Mux(_cfu.io.StallM.asBool,inst_tlb_exceptionE,inst_tlb_exceptionM))
    ...
    val HiInE = Mux(_id2ex.io2.HiLoWriteE === "b10".U,WriteCP0HiLoDataE,_muldiv.io.data_out) // b10 对应MTHI指令，通用寄存器到HI寄存器
    val LoInE = Mux(_id2ex.io2.HiLoWriteE === "b01".U,WriteCP0HiLoDataE,_muldiv.io.data_out) // b10 对应MTHI指令，通用寄存器到HI寄存器
...
// ff
    ...
    _commit_difftest.io.debug_pc       := _mem22wb.io.Pc_NextW
