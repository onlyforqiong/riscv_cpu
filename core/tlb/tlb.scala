...
// import examples.riscv_macros._
...
class tlb_write_or_read_port extends Bundle {
    ...
    def tlb_port_to_bundle_all_zero  = {
        vaddr := 0.U
        asid  := 0.U
        g     := 0.U
        // paddr := 0.U
...
class tlb(length:Int) extends Module with riscv_macros{

    val index_width = (log10(length)/log10(2)).toInt
    val io = IO(new Bundle{
        val   vaddr = Input(UInt(data_length.W))
        val   paddr = Output(UInt(data_length.W))
