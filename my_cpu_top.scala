...
class mycpu_top  extends RawModule with riscv_macros {
    ...
    val         axi_mem_port = IO(Vec(2,(new axi_ram_port)))
...
withClockAndReset(clk.asClock,(~aresetn).asAsyncReset) {
    ...
    if(tlb_on) {
        ...
        tlb.cp0_asid          := u_riscv_cpu.cp0_asid
        ...
        icache_first.cp0_asid := u_riscv_cpu.cp0_asid
        dcache_first.cp0_asid := u_riscv_cpu.cp0_asid
        ...
        u_riscv_cpu.cp0_tlb_write_data := tlb.tlb_read_port
        tlb.tlb_write_en     := u_riscv_cpu.tlb_write_en
        tlb.tlb_write_port   := u_riscv_cpu.cp0_tlb_read_data
        tlb.tlb_write_index  := u_riscv_cpu.tlb_write_index
        tlb.tlb_read_index   := u_riscv_cpu.tlb_read_index
    }else {
        icache_first.cp0_asid := 0.U//u_riscv_cpu.cp0_asid
        dcache_first.cp0_asid := 0.U//u_riscv_cpu.cp0_asid
        ...
        u_riscv_cpu.cp0_tlb_write_data := test_bundle_for_notlb
